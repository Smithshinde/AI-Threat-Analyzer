 ...
 * Backend API · Stack: Express · Groq (multi-model) · Supabase
 *
 * Routes:
 *   POST /api/analyze              — Standard analysis
 *   POST /api/analyze/file         — File upload analysis
 *   POST /api/analyze/stream       — Live streaming analysis (SSE)
 *   POST /api/analyze/compare      — Multi-model side-by-side comparison (SSE)
 *   GET  /api/scans                — Scan history
 *   GET  /api/scans/:id            — Single scan report
 *   GET  /api/scans/:id/matches    — Cross-scan IOC pattern matches
 *   PATCH /api/scans/:id/tags      — Update scan labels
 *   PATCH /api/scans/:id/share     — Toggle public sharing
 *   GET  /api/public/:id           — Public scan (no auth)
 *   GET  /api/chat/:scanId         — Chat history
 *   POST /api/chat/:scanId/stream  — Streaming AI chat (SSE)
...
// ── AI System prompts ────────────────────────────────────────────
const ANALYSIS_SYSTEM = `You are an elite threat intelligence analyst powering Clarion, an AI Security Intelligence Platform. You have deep expertise in:
...
const CHAT_SYSTEM = (scanContext) => `You are an expert threat intelligence analyst on the Clarion platform, reviewing a specific security incident.
...
async function saveToDb(userId, result, inputSize, model = 'llama-3.3-70b-versatile') {
  ...
  // Index IOCs for cross-scan pattern detection
...
app.post('/api/analyze', authenticate, async (req, res) => {
  ...
  try {
    ...
    console.error('[Clarion] Analysis error:', err.message);
...
app.post('/api/analyze/compare', authenticate, async (req, res) => {
  ...
  try {
    send({ type: 'status', message: 'Running both AI models in parallel…' });
...
app.use((err, _req, res, _next) => {
  ...
  console.error('[Clarion] Unhandled error:', err.message);
...
app.listen(PORT, () => {
  ...
  console.log(`  ██║     ███████╗██║  ██║██║  ██║██║╚██████╔╝██║ ╚████║`);
  console.log(`  ╚═╝     ╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝ ╚═════╝ ╚═╝  ╚═══╝\n`);
  console.log(`  AI Security Intelligence Platform`);
  console.log(`  API running at http://localhost:${PORT}`);
  console.log(`  Primary:   ${MODELS.primary.label}`);
  console.log(`  Secondary: ${MODELS.secondary.label}\n`);
});
